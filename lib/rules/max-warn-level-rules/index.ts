...
const createRule = ESLintUtils.RuleCreator((name: string) => 'eslint-plugin-config-itself/' + name)
...
export const rule = createRule<Options, 'maxWarnLevelRules'>({
  ...
  meta: {
    type: 'problem',
    docs: {
      description: 'Enforce a maximum number of warn level rules in .eslintrc.js',
    },
    messages: {
      maxWarnLevelRules: 'Too many warn level rules ({{total}}). Max is {{max}}.',
    },
    schema: [
      {
        type: 'object',
